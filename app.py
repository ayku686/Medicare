...
workout = pd.read_csv("workouts.csv")
...
symptoms_dict = {
    ...
    'history of alcohol consumption': 116, 'fluid overload.1': 117, 'blood in sputum': 118, 'prominent veins on calf': 119,
    'palpitations': 120, 'painful walking': 121, 'pus filled pimples': 122, 'blackheads': 123, 'scurring': 124,
    'skin peeling': 125, 'silver like dusting': 126, 'small dents in nails': 127, 'inflammatory nails': 128,
...
# User input with multiselect
selected_symptoms = st.multiselect(
    "Symptoms",
    options=list(symptoms_dict.keys()),
    help="Start typing a symptom and select from the list"
)

if selected_symptoms:
    disease, unknown_symptoms = get_predicted_value(selected_symptoms)
    
    if unknown_symptoms:
        st.write(f"Symptoms not found in the list: {', '.join(unknown_symptoms)}")
    else:
        st.write(f"Predicted Disease: {disease}")
        
        # Get additional information
        desc, precautions, medications, diets, workouts = helper(disease)
